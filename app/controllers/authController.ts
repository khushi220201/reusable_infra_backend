...
class AuthController {
  ...
  async register(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      await roleRepository.combineRoleCompany(
        company?.id,
        companyAdminRole?.id
      );
      await roleRepository.combineRoleCompany(
        company?.id,
        companyReadOnlyRole?.id
      );
      await roleRepository.combineRoleCompany(
        company?.id,
        companyAccountantRole?.id
      );
      ...
      const emailContent = getRegisterEmailTemplateInfra({ fullName,url });
      ...
      return DefaultResponse(
        res,
        200,
        "Registration Successfull, Kindly check your mail for verification."
      );
    } catch (err) {
      console.log(err);
      next(err);
   ...
   async verifyRegisteredEmail(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      const user = await authServices.verifyRegisteredUser(token,password,email);
